...
import ast.AbstractVariableDeclaration;
import ast.NodeType;
...
public class MethodVisitor extends ASTVisitor{
	...
	public boolean visit(SimpleName node) {
		...
		if(node.getParent().getNodeType()==40)
		{
			QualifiedName parent=(QualifiedName)node.getParent();
			SimpleName simple = parent.getName();
			
			
			
			statements.add(new AbstractFieldAccess(simple.getFullyQualifiedName(), "int","otherClass",true));
			return true;
