...
import ast.AbstractAnnotationMember;
import ast.AbstractEnumConstant;
import ast.AbstractField;
import ast.AbstractMethod;
import ast.AbstractParameter;
...
public class TypeVisitor extends ASTVisitor{
	...
	public boolean visit(AnnotationTypeMemberDeclaration node) {
		AbstractAnnotationMember annoMember = new AbstractAnnotationMember(node.getType().resolveBinding().getQualifiedName(), node.getName().getIdentifier());
	...
	public boolean visit(FieldDeclaration node) {
		...
		for (VariableDeclarationFragment vdf : (List<VariableDeclarationFragment>) node.fragments()) {
			fields2.add(new AbstractField(vdf.getName().getIdentifier(), type, modifiers, primitive));
		}
	...
	public boolean visit(MethodDeclaration node) {
		...
		List<Type> types = node.thrownExceptions();
		for (Type type : types) {
